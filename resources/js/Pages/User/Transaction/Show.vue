    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4 max-w-7xl mx-auto w-full">
            ...
            <div class="flex items-center gap-4">
                <Button variant="ghost" size="sm" @click="router.visit(route('transactions.index'))">
                    ...
                    <Card class="border-border bg-card">
                        <CardHeader>
                            <div class="flex items-center justify-between">
                                ...
                                <Badge variant="outline" :class="getStatusConfig(transaction?.status || transaction?.raw_status).color">
                                    {{ getStatusConfig(transaction?.status || transaction?.raw_status).label }}
                                </Badge>
                            </div>
                        </CardHeader>

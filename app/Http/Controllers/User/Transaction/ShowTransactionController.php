...
class ShowTransactionController extends Controller
{
    public function __invoke(Request $request, string $id): Response
    {
        ...
        // Tentar buscar como Transaction primeiro - com eager loading
        ...
        if ($transaction) {
            ...
            // Se não for Transaction, buscar como LiberpaySale pendente - com eager loading
            $liberpaySale = LiberpaySale::with('user')
                ->where('user_id', $user->id)
                ->find($id);
            ...
            if ($liberpaySale) {
                ...
                $fullpixSale = FullPixSale::with('user')
                    ->where('user_id', $user->id)
                    ->find($id);
                ...
                if (!$fullpixSale) {
                    ...
                    $fullpixSale = FullPixSale::with('user')
                        ->where('user_id', $user->id)
                        ->where('fullpix_transaction_id', $id)
                        ->first();
                }
        ...
        return Inertia::render('User/Transaction/Show', [
            'transaction' => $data,
        ]);
    }
